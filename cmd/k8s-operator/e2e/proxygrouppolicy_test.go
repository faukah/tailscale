...
func TestProxyGroupPolicy(t *testing.T) {
	...
	if err := createAndCleanupErr(t, kubeClient, egressService); err == nil {
	...
	if err := createAndCleanupErr(t, kubeClient, ingressService); err == nil {
	...
	if err := createAndCleanupErr(t, kubeClient, ingress); err == nil {
		t.Fatal("expected error when creating ingress Service")
	}
	...
	if err := createAndCleanupErr(t, kubeClient, egressService); err != nil {
		t.Fatalf("expected no error when creating egress Service, got %v", err)
	}

	if err := createAndCleanupErr(t, kubeClient, ingressService); err != nil {
		t.Fatalf("expected no error when creating ingress Service, got %v", err)
	}

	if err := createAndCleanupErr(t, kubeClient, ingress); err != nil {
		t.Fatalf("expected no error when creating Ingress, got %v", err)
	}
}
